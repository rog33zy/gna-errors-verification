...
import FaultyPaymentRequestsEntriesTable from "../UI/DuplicatePaymentRequestsTable/DuplicatePaymentRequestsTable";

const FaultyPaymentRequestsEntries = (props) => {
  ...
  function errorsListHandler() {
    for (let j = 0; j < shPaymentsRequestsSheetData.length; j++) {
      ...
      for (let i = 0; i < shPurchaseRequestsSheetData.length; i++) {
        ...
        try {
          total_weight_purchase_req =
            shPurchaseRequestsSheetDataRow.total_weight;
        } catch (error) {
          continue;
        }
    ...
    return shPaymentsRequestsSheetData;
  }
  return (
    <div>
      <FaultyPaymentRequestsEntriesTable
        rows={errorsListHandler()}
        title="Payment Requests With Errors"
        is_faulty_payments={true}
      />
    </div>
  );
};

export default FaultyPaymentRequestsEntries;
